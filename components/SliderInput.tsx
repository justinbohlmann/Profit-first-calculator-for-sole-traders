...
export const SliderInput: React.FC<SliderInputProps> = ({ id, label, note, min, max, step, value, onChange, format }) => {
  ...
  const displayValue = format === 'currency' ? value.toLocaleString('en-AU', {style: 'currency', currency: 'AUD', minimumFractionDigits: 0, maximumFractionDigits: 0}) : `${Math.round(value)}`;

  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-1">
        <label htmlFor={id} className="text-base font-normal text-[#111111]">{label}</label>
